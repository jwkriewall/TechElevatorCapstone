...
SELECT name, region FROM country WHERE continent = 'North America' OR continent = 'South America' 
EXCEPT SELECT name, region FROM country WHERE region = 'Caribbean';
...
SELECT name, continent, gnp, lifeexpectancy FROM country WHERE lifeexpectancy >= 70 AND gnp BETWEEN 1000000 AND 100000000;
-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
SELECT name, (gnp/population) FROM country WHERE continent = 'Europe';
